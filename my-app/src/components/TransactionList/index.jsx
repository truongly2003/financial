        ...
        {Object.entries(transactionsToRender).map(([date, data], index) => (
          <div key={index} className="mb-6 bg-white rounded-lg">
            ...
            {data.transactions.map((item, idx) => {
              const iconData = ICONS[item.icon] || { icon: '?', color: 'bg-gray-400' };
              return (
                <div key={idx} className="pb-4">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div className={`w-10 h-10 flex items-center justify-center ${iconData.color} rounded-full text-white`}>
                        {iconData.icon}
                      </div>
                     
                      <div>
                        <p className="text-gray-600 font-medium">{item.categoryName}</p>
                      </div>
                      <div className=''>
                        <p className='text-gray-600 font-normal'>{item.transactionDate}</p>
                      </div>
                    </div>
                    <p className={`font-bold ${item.categoryType === 'income' ? 'text-green-500' : 'text-red-500'}`}>
                      {`${item.categoryType === 'income' ? '+' : '-'}${item.amount.toLocaleString()} đ`}
                    </p>
                  </div>
                </div>
              );
